...
public:
    ...
    string select_siguptime_sqls(const string &user_name)
    {
        string sql = "SELECT signup_at from "+db+".tbl_user where user_name='"+user_name+"';";
        return sql;
    }
    ...
    string select_usr_tbl_file_sqls(const string &usrname)
    {
        string sql = "SELECT file_sha1,file_size,file_name,upload_at,last_update FROM "+db+".tbl_user_file WHERE user_name="+"'"+usrname+"';";
        return sql;
    }
    string geturl()const
    {
